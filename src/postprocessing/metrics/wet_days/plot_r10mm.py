...
def wet_days_plot(member="r1i1p1f1"):
    """
    3-row figure:
      Row 1: Absolute 30-year mean wet days (4 panels: mid/late x ssp126/ssp370)
      Row 2: Anomaly vs historical baseline (same 4 panels, diverging colormap)
      Row 3: Histogram of wet days per grid point (5 curves: historical + 4 combos)
    """

    VAR = "heavy_precipitation_days_index_per_time_period"

    # --- Load data ---
    ds126 = xr.open_dataset(f"/scratch/project_465002687/ec_earth/metrics/r10mm_hclim_{member}_ssp126.nc")
    ds370 = xr.open_dataset(f"/scratch/project_465002687/ec_earth/metrics/r10mm_hclim_{member}_ssp370.nc")
    ...
    for i, area in enumerate(area_keys):
        ...
        axes_hist[i].set_xlabel("Wet days per year")
        axes_hist[i].set_ylabel("Log Density")
        axes_hist[i].set_title(f"Distribution of wet days across {area} region", fontsize=12)
        axes_hist[i].set_yscale("log")
        axes_hist[i].legend()
        axes_hist[i].set_xlim(left=0, right=366)

    # --- Final touches ---
    plt.suptitle("Annual Wet Days (PR > 10mm)", fontsize=18, fontweight="bold")
    return fig
