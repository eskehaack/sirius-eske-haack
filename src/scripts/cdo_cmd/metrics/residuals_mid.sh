...
# Compute time mean for mid century ec earth data
...
if [ ! -e "${REGRID_OUTFILE}" ]; then
    echo "Computing time mean for mid century ec earth data"
    cdo timmean -selyear,2020/2050 ${INFILE} ${REGRID_OUTFILE}
else
    echo "Output file ${REGRID_OUTFILE} already exists. Skipping time mean computation."
fi

# Compute time mean for mid century hclim data
...
if [ ! -e "${HCLIM_OUTFILE}" ]; then
    echo "Computing time mean for mid century hclim data"
    cdo timmean -selyear,2020/2050 ${INFILE} ${HCLIM_OUTFILE}
else
...
if [ ! -e "${OUTFILE}" ]; then
    echo "Computing residuals for mid century data"
